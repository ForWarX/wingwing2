...
if (isset($_REQUEST['is_ajax']) === false || $_REQUEST['is_ajax'] == ''){
...
else
{
	...
	if ($_REQUEST['act'] == 'autoCompleteSearchAjax'){
		require(ROOT_PATH . 'includes/cls_json.php');
		
		//$sql = 'SELECT * FROM ' . $ecs->table('goods') . " WHERE 'goods_name' LIKE '%" . $_REQUEST['key'] . "%'";
		/*$sql = 'SELECT * FROM ' . $ecs->table('goods') . ' WHERE "goods_name"" LIKE "%bear%" ';
		
        $res = $GLOBALS['db']->getAll($sql);*/
		$val = $_REQUEST['term'];
        $sql = 'SELECT * FROM ' . $ecs->table('goods') . " WHERE goods_name LIKE '%$val%' ORDER BY click_count DESC LIMIT 10";
        $res = $db->query($sql);
        while ($row = $db->FetchRow($res))
        {
            $name[] = $row['goods_name'];
        ...
        $sql = 'SELECT * FROM ' . $ecs->table('brand') . " WHERE brand_name LIKE '%$val%' LIMIT 5 ";
        $res = $db->query($sql);
        while ($row = $db->FetchRow($res))
        {
