...
/**
 * 取得是否多次减免信息
 * @return  boolean  是否多次减免信息
 */
function is_multi_discount()
{
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('shop_config') .
            " WHERE id = 428";
	$return = $GLOBALS['db']->getRow($sql);
	if ($return['value'] == 1){return true;}
	else {return false;}
}
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
		...
		$order['formated_surplus_cny']    = price_format($order['surplus_cny'], false);
...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['extension_code'] != 'group_buy')
    {
        ...
        $total['discount'] = $discount_number;
	...
	/* 人民币余额 */
    $order['surplus_cny'] = $order['surplus_cny'] > 0 ? $order['surplus_cny'] : 0;
	...
	if ($total['amount'] > 0)
    {
		...
		if (isset($order['surplus_cny']) && $order['surplus_cny'] > $total['amount'])
        {
            $order['surplus_cny'] = $total['amount'];
            $total['amount']  = 0;
			$order['surplus'] = 0;
        }
        else
        {
            $total['amount'] -= $order['surplus_cny'];
        }
    }
    else
    {
    ...
    $total['surplus_cny'] = $order['surplus_cny'];
	$total['surplus_cny_formated'] = price_format($order['surplus_cny'], false);
    ...
    if ($total['amount'] > 0 && $max_amount > 0 && $order['integral'] > 0)
    {
		...
		if(abs($max_amount - $integral_money)<=0.01){$integral_money = $max_amount;}
	...
	$total['goods_price_rmb'] = round($total['amount'] * $xe, 1);
...
function bonus_info($bonus_id, $bonus_sn = '')
{
	 ...
	 if($row['send_type'] == 4) // phpsir 如果是第4类型红包，那么就找一个未使用的红包,这种红包可被多次使用，不限制每人使用次数
    {
        ...
        $allow_used_bonus_num = 1; // 最大允许使用次数
...
function value_of_integral($integral)
{
	...
	$scale = round(100/floatval($xe), 4);
	//$scale = floatval($GLOBALS['_CFG']['integral_scale']);

    return $scale > 0 ? round(($integral / 1000) * $scale, 2) : 0;
}
...
function integral_of_value($value)
{
    ...
    return $scale > 0 ? round($value / $scale * 1000) : 0;
}
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0)
{
    ...
    if (1 == $refund_type)
    {
        log_account_change($user_id, 0, $amount, 0, 0, 0, $change_desc);
...
function check_consignee_info($consignee, $flow_type)
{
	if (exist_real_goods(0, $flow_type))
    {
        ...
        if ($res)
        {
			...
			elseif ($consignee['country'] == 1 && empty($consignee['consignee_id']))
			{
				$res = false;
			}
			elseif ($consignee['country'] == 1)
			{
				if($consignee['have_id_front'] != 1 || $consignee['have_id_back'] != 1)
				{
					$res = false;
...
function flow_order_info()
{
	...
	if (!isset($order['surplus_cny']))
    {
        $order['surplus_cny'] = 0;  // 初始化人民币余额
    }
...
/**
 * 计算折扣：根据购物车和优惠活动
 * @param   boolean $is_multi_discount 自动计算多次减免，如果金额超过减免规则
 * @return  float   折扣
 */
function compute_discount($is_multi_discount=false)
{
    /* 查询优惠活动 */
    $now = gmtime();
    ...
    /* 查询购物车商品 */
    $sql = "SELECT c.goods_id, c.goods_price, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id " .
    ...
    foreach ($favourable_list as $favourable)
    {
        ...
        elseif ($favourable['act_range'] == FAR_GOODS)
        {
            foreach ($goods_list as $goods)
            {
                ...
                {
                    $total_amount += $goods['subtotal'];
                    $unit_goods_price = $goods['goods_price'];//单价
                }
            }
        }
        else
        ...
        {
			/*foreach (){
			$unit_goods_price  = $goods_list[0]['goods_price'];//单个价格
			
			$total_goods_price = $goods_list[0]['subtotal'];//总共价格
			
			$quantity_of_goods = $total_goods_price / $unit_goods_price;//产品数量
			}*/

			$times_need_to_discount = 1;
			if ($is_multi_discount === true){ $times_need_to_discount = floor($total_amount / $favourable['min_amount']);}//优惠次数
            ...
            elseif ($favourable['act_type'] == FAT_PRICE)
            {
				...
				for ($i = 0; $i < $times_need_to_discount; $i++){
 ...
 * @return  float   折扣
 */
function compute_discount_with_favourable_list_mix($is_multi_discount=false)
{
    /* 查询优惠活动 */
    $now = gmtime();
    $user_rank = ',' . $_SESSION['user_rank'] . ',';
    $sql = "SELECT *" .
            ...
            " AND CONCAT(',', user_rank, ',') LIKE '%" . $user_rank . "%'" .
            " AND act_type " . db_create_in(array(FAT_DISCOUNT, FAT_PRICE));
    $favourable_list_mix = $GLOBALS['db']->getAll($sql);
    if (!$favourable_list_mix)
    {
        return 0;
    }
    ...
    /* 循环计算每个优惠活动的折扣 */
    foreach ($favourable_list_mix as $favourable)
    {
        $total_amount = 0;
        if ($favourable['act_range'] == FAR_ALL)
        {
        ...
        {	
            ...
            foreach ($goods_list as $goods)
            {
                if (strpos(',' . $favourable['act_range_ext'] . ',', ',' . $goods['goods_id'] . ',') !== false)
                {
                    $total_amount += $goods['subtotal'];
                    $unit_goods_price = $goods['goods_price'];//单价
					$unit_quantity = floor($total_amount / $unit_goods_price);
...
function get_give_integral()
{
        ...
        if ($_SESSION['user_rank'] ==3){
			$sql = "SELECT SUM(c.goods_number * IF(g.give_integral > -1, g.give_integral, c.goods_price))" .
					...
					"AND c.rec_type = 0 " .
					"AND c.is_gift = 0";
			$goods_amounts_in_rmb = floatval($GLOBALS['db']->getOne($sql)) * floatval($xe);
			$integral_given = floor($goods_amounts_in_rmb / 100) * 3;
			return $integral_given;
		}
		else{
			return 0;	
		}
...
function integral_to_give($order)
{
    ...
    else
    {
		...
		if (intval($user_rank) == 3){
        	...
        	$base_integral_array = $GLOBALS['db']->getRow($sql);
			
			$goods_amount_in_rmb = floatval($base_integral_array['custom_points']) * floatval($xe);
			
			$final_integral = floor($goods_amounts_in_rmb / 100) * 3;
			return array('custom_points' => $final_integral, 'rank_points' => $base_integral_array['rank_points']);
