 ...
 * @return  string
 */
function insert_comments_list($arr)
{
    ...
    $GLOBALS['smarty']->assign('tab_active_class_normal',           'active');
    $GLOBALS['smarty']->assign('active_tab_value',           'all_comments');
 ...
 * @return  string
 */
function insert_comments_form($arr)
{
    $need_cache = $GLOBALS['smarty']->caching;
    $need_compile = $GLOBALS['smarty']->force_compile;
	...
	$bought  = is_user_bought ($user_id,$arr['id']);
	$comment = is_user_comment($user_id,$arr['id']);
	$comment = $comment['result'];
	
	$bought = true;$comment = false;

	$show_form = 0;
	if (($user_id != 0 && $bought === true && $comment === false) || ($arr['ready_comment'] == 'comment')){

			if ($arr['ready_comment_id'] != ''){
				 $GLOBALS['smarty']->assign('ready_comment_id',        $arr['ready_comment_id']);
			}

		 $show_form = 1;
	}
	
	$GLOBALS['smarty']->assign('show_form',        $show_form);
	
	$val = $GLOBALS['smarty']->fetch('library/comments_form.lbi');
...
// 判断用户是否买过该产品 //
function is_user_bought($user_id,$goods_id){
	...
	foreach ($goods_arr as $key => $value){
		   ...
		   $order_value = $GLOBALS['db']->getAll($sql);
		   $order_arr[] = $order_value[0];
	}
	
	foreach ($order_arr as $key => $value){
		   if ($user_id == $value['user_id'] ){$bought = true;break;}
	}
	return $bought;
}

// 判断用户是否评论过该产品 //
function is_user_comment($user_id,$goods_id){
	if ($user_id == ''){return false;}
	if ($goods_id == ''){return false;}
	
	$comment = array();
	$sql = 'SELECT user_id, comment_id ' .
	...
	foreach ($user_arr as $key => $value){
		   if ($user_id == $value['user_id'] ){$comment['result'] = true;$comment['comment_id'] = $value['comment_id'];break;}
	}
	//showr($comment);
	return $comment;
}
