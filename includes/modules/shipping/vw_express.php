...
$shipping_lang = ROOT_PATH.'languages/' .$GLOBALS['_CFG']['lang']. '/shipping/vw_express.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 配送方式的描述 */
    $modules[$i]['desc']    = 'vw_express_desc';
    ...
    /* 打印单背景 */
    $modules[$i]['print_bg'] = '/images/receipt/dly_vw_express.jpg';
...
class vw_express
{
    ...
    function calculate($goods_weight, $goods_amount, $goods_number)
    {
        ...
        {	
			...
			@$lb_goods_weight = $goods_weight * 2.2;
            ...
            {
                if (($lb_goods_weight) > 1)
                {
					if (($lb_goods_weight) <= 5)
					{
						$fee += (ceil((($lb_goods_weight) - 1))) * $this->configure['step_fee'];
					}
					else
					{
						@$left_weight = $lb_goods_weight - 1;
						@$round_weight = round($left_weight);
						if ($left_weight > $round_weight)
						{
							$fee += (round((($lb_goods_weight) - 1))) * $this->configure['step_fee'];
							$fee += 0.5 * $this->configure['step_fee'];
						}
						else
						{
							$fee += (round((($lb_goods_weight) - 1))) * $this->configure['step_fee'];
				...
				if (($lb_goods_weight) < 1 && ($lb_goods_weight) > 0){
					$fee += $this->configure['step_fee'];
				}
				
            }
			if($goods_weight == 0){$fee = 0;}
            return $fee;
        }
    }
    ...
    function query($invoice_sn)
    {
		...
		$str= '<form></form>';
		/*中加TN开头*/
		if (0 === strpos($invoice_sn, 'TN')) {
			$multi = explode("<br>",$invoice_sn);
			foreach ($multi as $sep){
				$str .= '<form style="margin:0px" method="post" '.
				'action="zj_helper.php" name="queryForm_' .$sep. '" target="_blank">'.
				'<input type="hidden" name="action" value="add_foobar" />'.
				'<input type="hidden" name="data" value="' .$sep. '" />'.
				'<a href="javascript:document.forms[\'queryForm_' .$sep. '\'].submit();">' .$sep. '</a>'.
				'</form>';
			}
		}
		/*AAE*/
		else if(0 === strpos($invoice_sn,'AAE') || 0 === strpos($invoice_sn,'1')){
			$multi = explode("<br>",$invoice_sn);
			foreach ($multi as $sep){
				if(0 === strpos($sep,'AAE')){
					$sep = substr($sep,3);
				...
				$str .= '<form style="margin:0px" method="post" '.
				'action="http://cn.aaeweb.com/track/default.asp" name="queryForm_' .$sep. '" target="_blank">'.
				'<input type="hidden" name="danhao" value="' .$sep. '" />'.
				'<a href="javascript:document.forms[\'queryForm_' .$sep. '\'].submit();">' .$sep. '</a>'.
				'</form>';
			}
		...
		/*美通*/
			...
			$str .= '<form style="margin:0px" method="post" '.
				'action="http://member.valueway.net/cgi-bin/GInfo.dll?EmmisTrack" name="queryForm_' .$invoice_sn. '" target="_blank">'.
				'<input type="hidden" name="cno" value="' .str_replace("<br>","\n",$invoice_sn). '" />'.
