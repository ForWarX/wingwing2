...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
                $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, c.show_banner_in_home_page, c.show_banner_in_category_page, c.is_event_area, COUNT(s.cat_id) AS has_children ".
...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        {
            $arr['integrate_code'] = 'ecshop'; // 默认的会员整合插件为 PBCC
        }
 ...
 * @return  void
 */
function log_account_change($user_id, $currency_type = 0, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
	...
	if ($currency_type == 0){
		...
		$user_money_cny = round($user_money * get_exchange_rate('CNY', 'CAD'));
		$sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
				" SET user_money_cny = user_money_cny + ('$user_money_cny')," .
				" frozen_money = frozen_money + ('$frozen_money')," .
				" rank_points = rank_points + ('$rank_points')," .
				" pay_points = pay_points + ('$pay_points')" .
				" WHERE user_id = '$user_id' LIMIT 1";	
...
function get_top_brands(){
	$sql = "SELECT b.brand_name, b.brand_id, t.total, b.brand_logo 
			FROM ".$GLOBALS['ecs']->table('brand'). " AS b, (SELECT brand_id, sum(click_count) AS total FROM ".$GLOBALS['ecs']->table('goods')." GROUP BY brand_id) AS t 
			WHERE b.brand_id = t.brand_id AND (b.brand_name REGEXP '^[a-zA-Z]+') AND b.is_show = 1
...
function get_exchange_rate($base_cur,$trans_cur){
	$sql = "SELECT rate FROM ".$GLOBALS['ecs']->table('xe_rate')." WHERE base_cur = '" . $base_cur . "' AND trans_cur = '" . $trans_cur . "'";
