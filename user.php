...
'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email',
'clear_history','qpassword_name', 'get_passwd_question', 'check_answer', 'send_pwd_email_ajax', 'get_question_username_ajax', 'check_answer_ajax');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'pcoin_deposit', 'account_log', 'account_detail', 'act_account', 'act_pcoin_account', 'pay', 
'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 
...
/* 密码找回-->根据注册用户名取得密码提示问题界面ajax */
elseif ($action == 'get_question_username_ajax')
{
	include_once('includes/cls_json.php');
    $json = new JSON;
	...
	$result = array('error' => $error, 'content' => '');
	die($json->encode($result));
}
...
elseif ($action == 'get_passwd_question')
{
		...
		if (empty($_POST['user_name']))
		{
			show_message_user($_LANG['no_passwd_question'], $_LANG['back_home_lnk'], './', 'info', false);
		}
		else
		{
			$user_name = trim($_POST['user_name']);
...
elseif ($action == 'check_answer')
{
    ...
    if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0)
    {
        if (empty($_POST['captcha']))
        {
            show_message($_LANG['invalid_captcha'], $_LANG['back_retry_answer'], 'user.php?act=qpassword_name', 'error', false);
        }
    ...
    {
        show_message($_LANG['wrong_passwd_answer'], $_LANG['back_retry_answer'], 'user.php?act=qpassword_name', 'info', false);
    }
    else
    {
...
/* 密码找回-->根据提交的密码答案进行相应处理 */
elseif ($action == 'check_answer_ajax')
{
	include_once('includes/cls_json.php');
	$json = new JSON;
...
/* 处理 ajax 的发送密码修改确认邮件 */
elseif ($action == 'send_pwd_email_ajax')
{
    include_once(ROOT_PATH . 'includes/lib_passport.php');
    include_once('includes/cls_json.php');
    $json = new JSON;
	...
	if ($user_info && $user_info['email'] == $email)
    {
        ...
        {
            //发送邮件出错
            $error = 0;//0:发送邮件出错 1:用户名与邮件地址不匹配
        }
    }
    else
    {
        //用户名与邮件地址不匹配
		$error = 1;//0:发送邮件出错 1:用户名与邮件地址不匹配
    }
...
elseif ($action == 'act_edit_password')
{
    ...
    {
        show_message_user($_LANG['passport_js']['password_shorter']);
    }
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
    {
        ...
        if ($user->edit_user(array('username'=> (empty($code) ? $_SESSION['user_name'] : $user_info['user_name']), 'old_password'=>$old_password, 'password'=>$new_password), empty($code) ? 0 : 1))
        {
			...
			$db->query($sql);
            $user->logout();
            show_message_user($_LANG['edit_password_success'], $_LANG['relogin_lnk'], 'user.php?act=login', 'info', false);
        }
        else
        {
            show_message_user($_LANG['edit_password_failure'], $_LANG['back_page_up'], '', 'info', false);
        }
    }
    else
    {
        show_message_user($_LANG['edit_password_failure_due_time'], $_LANG['back_retry_answer'], 'user.php?act=login', 'info', false);
    }
 ...
 * @return  void
 */
function show_message_user($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
    assign_template();
	...
	$user_login_background_image = 'Sharon_&_Nikki_McCutcheon.jpg';
	$GLOBALS['smarty']->assign('user_login_background_image', $user_login_background_image);
    $GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->display('message_user.dwt');
