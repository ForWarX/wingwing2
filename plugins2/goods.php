...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        /* 图片列表 */
        $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_id = '$goods[goods_id]' order by img_order";
		...
		/* 产品功效 */
        $sql = "SELECT goods_func_id, goods_func_content, goods_func_content_en, goods_func_order FROM " . $ecs->table('goods_func') . " WHERE goods_id = '$goods[goods_id]' order by goods_func_order";
        $func_list = $db->getAll($sql);
		
		/* 特色吃法 */
        $sql = "SELECT goods_use_id, goods_use_title, goods_use_title_en, goods_use_content, goods_use_content_en, goods_use_order FROM " . $ecs->table('goods_use') . " WHERE goods_id = '$goods[goods_id]' order by goods_use_order";
        $use_list = $db->getAll($sql);
		
		/* 特色吃法图片 */
		$uri = $ecs->url();
		$use_img = '';
		$sql = "SELECT use_img FROM " . $ecs->table('goods') . " WHERE goods_id = '$goods[goods_id]'";
        $use_img = $db->getAll($sql);
        $smarty->assign('use_img', $use_img);
	...
	print $goods['goods_desc_en'];
	print 1234;
    /* 创建 html editor */
    //create_html_editor('goods_desc', $goods['goods_desc'], $goods['goods_desc_en']);
    create_html_editor('FCKeditor', $goods['goods_desc'], $goods['goods_desc_en']);
    ...
    $smarty->assign('show_in_thumbnail', 1);
    $smarty->assign('show_in_detail', 1);
    ...
    $smarty->assign('func_list', $func_list);
    $smarty->assign('use_list', $use_list);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (isset($_FILES['goods_img']['error'])) // php 4.2 版本才支持 error
    {
		...
		// 特色吃法图片
        if (isset($_FILES['use_img']))
        {
            if ($_FILES['use_img']['error'] == 0)
            {
                if (!$image->check_img_type($_FILES['use_img']['type']))
                {
                    sys_msg($_LANG['invalid_use_img'], 1, array(), false);
                }
            }
            elseif ($_FILES['use_img']['error'] == 1)
            {
                sys_msg(sprintf($_LANG['use_img_too_big'], $php_maxsize), 1, array(), false);
            }
            elseif ($_FILES['use_img']['error'] == 2)
            {
                sys_msg(sprintf($_LANG['use_img_too_big'], $htm_maxsize), 1, array(), false);
            }
        }
    }
    /* 4.1版本 */
	...
	$use_img          = '';  // 特色吃法图片
	$original_use_img     = '';  // 初始化原始特色吃法图片
    $old_original_use_img = '';  // 初始化原始特色吃法图片旧图
	
	// 如果上传了特色吃法图片，相应处理
    if ($_FILES['use_img']['tmp_name'] != '' && $_FILES['use_img']['tmp_name'] != 'none')
    {
        if ($_REQUEST['goods_id'] > 0)
        {
            /* 删除原来的图片文件 */
            $sql = "SELECT use_img, original_use_img " .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
            $row = $db->getRow($sql);

            if ($row['use_img'] != '' && is_file('../' . $row['use_img']))
            {
                @unlink('../' . $row['use_img']);
            }
            if ($row['original_use_img'] != '' && is_file('../' . $row['original_use_img']))
            {
                /* 先不处理，以防止程序中途出错停止 */
                //$old_original_use_img = $row['original_use_img']; //记录旧图路径
            }
            /* 清除原来商品图片 */
            if ($proc_thumb === false)
            {
                get_image_path($_REQUEST[goods_id], $row['use_img'], false, 'goods', true);
            }
        }

        if (empty($is_url_use_img))
        {
            $original_use_img   = $image->upload_image($_FILES['use_img']); // 原始特色吃法图片
        }
        elseif (copy(trim($_POST['use_img_url']), ROOT_PATH . 'temp/' . basename($_POST['use_img_url'])))
        {
            $original_use_img = 'temp/' . basename($_POST['use_img_url']);
        }

        if ($original_use_img === false)
        {
            sys_msg($image->error_msg(), 1, array(), false);
        }
        $use_img      = $original_use_img;   // 特色吃法图片

        /* 复制一份相册图片 */
        /* 添加判断是否自动生成相册图片 */
        /*if ($_CFG['auto_generate_gallery'])
        {
            $img        = $original_use_img;   // 相册图片
            $pos        = strpos(basename($img), '.');
            $newname    = dirname($img) . '/' . $image->random_filename() . substr(basename($img), $pos);
            if (!copy('../' . $img, '../' . $newname))
            {
        ...
        // 如果系统支持GD，缩放商品图片，且给商品图片和相册图片加水印
        if ($image->gd_version() > 0 && $image->check_img_function($_FILES['goods_img']['type']) && $_FILES['goods_img']['width'] > $GLOBALS['_CFG']['image_width'] && $_FILES['goods_img']['height'] > $GLOBALS['_CFG']['image_height'] || $is_url_goods_img)
		{
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_en, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, goods_desc_en, add_time, last_update, goods_type, rank_integral, suppliers_id, use_img, original_use_img)" .
                "VALUES ('$_POST[goods_name]', '$_POST[goods_name_en]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '$_POST[goods_desc_en]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$use_img', '$original_use_img')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_en, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, goods_desc_en, add_time, last_update, goods_type, extension_code, rank_integral, use_img, original_use_img)" .
                "VALUES ('$_POST[goods_name]', '$_POST[goods_name_en]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '$_POST[goods_desc_en]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$use_img', '$original_use_img')";
        }
    }
    else
    {
        /* 如果有上传图片，删除原来的商品图 */
        $sql = "SELECT goods_thumb, goods_img, original_img, use_img, original_use_img " .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);
		...
		if ($use_img && $row['use_img'])
        {
            @unlink(ROOT_PATH . $row['use_img']);
            @unlink(ROOT_PATH . $row['original_use_img']);
        }
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
				...
				"goods_desc_en = '$_POST[goods_desc_en]', " .
                "goods_sn = '$goods_sn', " .
                "cat_id = '$catgory_id', " .
                "brand_id = '$brand_id', " .
        ...
        if ($use_img)
        {
            $sql .= "use_img = '$use_img', original_use_img = '$original_use_img', ";
        }
        if ($code != '')
        {
    ...
    $use_img = reformat_image_name('use_img', $goods_id, $use_img, 'use');
	$original_use_img = reformat_image_name('use_img', $goods_id, $original_use_img, 'source');
    if ($goods_img !== false)
    {
	...
	if ($use_img !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods') . " SET use_img = '$use_img' WHERE goods_id='$goods_id'");
    }
	
	if ($original_use_img !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods') . " SET original_use_img = '$original_use_img' WHERE goods_id='$goods_id'");
    }
    ...
    if (isset($img))
    {
        ...
        $sql = "INSERT INTO " . $ecs->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original,img_order, img_show_in_thumbnail, img_show_in_detail) " .
                "VALUES ('$goods_id', '$gallery_img', '', '$gallery_thumb', '$img', '', 1, 1)";
        $db->query($sql);
    }
//showr($_POST);
    /* 处理相册图片 */
    handle_gallery_image($goods_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file'], $_POST['img_order'],$_POST['img_show_in_thumbnail'],$_POST['img_show_in_detail']);
    ...
    {	
        ...
        {
			if(isset($_POST['old_img_show_in_thumbnail'][$img_id]) === false || $_POST['old_img_show_in_thumbnail'][$img_id] == ''){
				$_POST['old_img_show_in_thumbnail'][$img_id] = 0;
			}
			if(isset($_POST['old_img_show_in_detail'][$img_id]) === false || $_POST['old_img_show_in_detail'][$img_id] == ''){
				$_POST['old_img_show_in_detail'][$img_id] = 0;
			}
	...
	/* 编辑时处理相册图片是否在缩略图显示 */
    if (!$is_insert && isset($_POST['old_img_show_in_thumbnail']))
    {//print "vvvv";showr($_POST);
        foreach ($_POST['old_img_show_in_thumbnail'] AS $img_id => $img_show_in_thumbnail)
        {	
            $sql = "UPDATE " . $ecs->table('goods_gallery') . " SET img_show_in_thumbnail = '$img_show_in_thumbnail' WHERE img_id = '$img_id' LIMIT 1";
            $db->query($sql);
        }
    }
	
	/* 编辑时处理相册图片是否在细节图显示 */
    if (!$is_insert && isset($_POST['old_img_show_in_detail']))
    {//print "vvvv";showr($_POST);
        foreach ($_POST['old_img_show_in_detail'] AS $img_id => $img_show_in_detail)
        {	
            $sql = "UPDATE " . $ecs->table('goods_gallery') . " SET img_show_in_detail = '$img_show_in_detail' WHERE img_id = '$img_id' LIMIT 1";
            $db->query($sql);
        }
    }
	...
	$count_old_func = 0;/*print '1';showr($_REQUEST);*/
	if(!empty($_POST['old_func_content'])){
		//print 'e';
		//print(count($_POST['old_func_content']));
		$count_old_func = count($_POST['old_func_content']);
		
	}
	
	handle_func($goods_id, $_POST['func_order'],$_POST['func_content'],$_POST['func_content_en'],$count_old_func);

	/* 编辑时处理产品功效顺序 */
    if (!$is_insert && isset($_POST['old_func_order']))
    {	
        foreach ($_POST['old_func_order'] AS $func_id => $func_order)
        {
            $sql = "UPDATE " . $ecs->table('goods_func') . " SET goods_func_order = '$func_order' WHERE goods_func_id = '$func_id' LIMIT 1";
            $db->query($sql);
        }
    }
	
	/* 编辑时处理产品功效内容 */
    if (!$is_insert && isset($_POST['old_func_content']))
    {	
        foreach ($_POST['old_func_content'] AS $func_id => $func_content)
        {
            $sql = "UPDATE " . $ecs->table('goods_func') . " SET goods_func_content = '$func_content' WHERE goods_func_id = '$func_id' LIMIT 1";
            $db->query($sql);
        }
    }
	
	/* 编辑时处理产品功效内容英文 */
    if (!$is_insert && isset($_POST['old_func_content_en']))
    {	
        foreach ($_POST['old_func_content_en'] AS $func_id => $func_content_en)
        {
            $sql = "UPDATE " . $ecs->table('goods_func') . " SET goods_func_content_en = '$func_content_en' WHERE goods_func_id = '$func_id' LIMIT 1";
            $db->query($sql);
        }
    }
	//===================================== 产品功效结束 ==================================================================//
	...
	//====================================== 特色吃法 ==================================================================//
	//// 特色吃法 ////
	handle_use($goods_id, $_POST['use_order'],$_POST['use_title'],$_POST['use_title_en'],$_POST['use_content'],$_POST['use_content_en']);

	/* 编辑时处理特色吃法顺序 */
    if (!$is_insert && isset($_POST['old_use_order']))
    {	
        foreach ($_POST['old_use_order'] AS $use_id => $use_order)
        {
            $sql = "UPDATE " . $ecs->table('goods_use') . " SET goods_use_order = '$use_order' WHERE goods_use_id = '$use_id' LIMIT 1";
            $db->query($sql);
        }
    }
	
	/* 编辑时处理特色吃法标题 */
    if (!$is_insert && isset($_POST['old_use_title']))
    {	
        foreach ($_POST['old_use_title'] AS $use_id => $use_title)
        {	
            $sql = "UPDATE " . $ecs->table('goods_use') . " SET goods_use_title = '$use_title' WHERE goods_use_id = '$use_id' LIMIT 1";
            $db->query($sql);
        }
    }
	
	/* 编辑时处理特色吃法标题英文 */
    if (!$is_insert && isset($_POST['old_use_title_en']))
    {	
        foreach ($_POST['old_use_title_en'] AS $use_id => $use_title_en)
        {	
            $sql = "UPDATE " . $ecs->table('goods_use') . " SET goods_use_title_en = '$use_title_en' WHERE goods_use_id = '$use_id' LIMIT 1";
            $db->query($sql);
        }
    }
	
	/* 编辑时处理特色吃法内容 */
    if (!$is_insert && isset($_POST['old_use_content']))
    {	
        foreach ($_POST['old_use_content'] AS $use_id => $use_content)
        {	
            $sql = "UPDATE " . $ecs->table('goods_use') . " SET goods_use_content = '$use_content' WHERE goods_use_id = '$use_id' LIMIT 1";
            $db->query($sql);
        }
    }
	
	/* 编辑时处理特色吃法内容英文 */
    if (!$is_insert && isset($_POST['old_use_content_en']))
    {	
        foreach ($_POST['old_use_content_en'] AS $use_id => $use_content_en)
        {
            $sql = "UPDATE " . $ecs->table('goods_use') . " SET goods_use_content_en = '$use_content_en' WHERE goods_use_id = '$use_id' LIMIT 1";
            $db->query($sql);
        }
    }
...
//-- 删除吃法
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_use')
{
	check_authz_json('goods_manage');

    $use_id = empty($_REQUEST['use_id']) ? 0 : intval($_REQUEST['use_id']);

    /* 删除数据 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('goods_use') . " WHERE goods_use_id = '$use_id' LIMIT 1";
    $GLOBALS['db']->query($sql);

    clear_cache_files();
    make_json_result($use_id);
...
function put_banner_xml($flashdb,$file_name)
{
    if (!empty($flashdb))
    {
        ...
        $xml .= '<item item_url="' . $flashdb['src'] . '" link="' . $flashdb['url'] . '" text="' . $flashdb['text'] . '" sort="' . $flashdb['sort'] . '" position="' . $flashdb['position'] . '" price="' . $flashdb['price'] . '" title="' . $flashdb['title'] . '"/>';
